...
module layer_norm #(
    ...
    // [Signal Declarations same as before...]
    ...
    reg signed [STAT_WIDTH-1:0]    delayed_mean [0:1];
    wire [BEAT_WIDTH-1:0] fn_data;
    wire                  fn_valid, fn_last, fn_ready;

    // [NEW] Parameter FIFO Interconnects
    wire [127:0] params_pack_in;
    wire [127:0] params_pack_out;
    wire         params_fifo_valid_out;
    wire         params_fifo_ready_in;
    ...
    beat_fifo #(
        ...
        .m_axis_tvalid(df_valid), .m_axis_tready(df_read_en),
        .fifo_count()
    );
    ...
    // [NEW] PARAMETER FIFO
    // ---------------------------------------------------------
    // Pack: {Beta(32), Gamma(32), InvSqrt(32), Mean(32)} = 128 bits
    // We store cfg_gamma/beta HERE to ensure we use the config 
    // that existed when the stats were calculated.
    assign params_pack_in = {cfg_beta, cfg_gamma, peano_inv_sqrt, delayed_mean[1]};

    beat_fifo #(
        .DATA_WIDTH(128), .DEPTH(16), .RAM_STYLE("distributed")
    ) u_params_fifo (
        .clk(clk), .aresetn(aresetn),
        // Write Side (From Stats Engine)
        .s_axis_tdata(params_pack_in),
        .s_axis_tlast(1'b0),          // Unused
        .s_axis_tvalid(peano_valid),
        .s_axis_tready(),             // Assuming Depth 16 is sufficient for flight time
        
        // Read Side (To Final Norm)
        .m_axis_tdata(params_pack_out),
        .m_axis_tlast(),
        .m_axis_tvalid(params_fifo_valid_out),
        .m_axis_tready(params_fifo_ready_in) // Controlled by Final Norm
    );
    // ---------------------------------------------------------
    ...
    final_norm_calc #(
        ...
        .s_axis_last(df_last), 
        .s_axis_ready(df_read_en),
        
        // [MODIFIED] Inputs now come from FIFO
        .mean_val    (params_pack_out[31:0]), 
        .inv_sqrt_val(params_pack_out[63:32]),
        .gamma_val   (params_pack_out[95:64]), 
        .beta_val    (params_pack_out[127:96]),
        
        // [MODIFIED] Handshake
        .params_valid(params_fifo_valid_out),
        .params_ready(params_fifo_ready_in), // Drives FIFO read enable
