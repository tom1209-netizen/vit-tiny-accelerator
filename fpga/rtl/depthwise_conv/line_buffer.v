...
module line_buffer #(
    ...
    output wire [INPUT_WIDTH-1:0] rd_data_3   // Line buffer 3 data
);
    localparam MAX_BEATS_ROW = MAX_WIDTH * (MAX_CHANNELS / LANES);  // 28 * 16 = 448

    // Line Buffers - 4 BRAMs
    ...
    (* ram_style = "block" *) reg [INPUT_WIDTH-1:0] line_buf_3[0:MAX_BEATS_ROW-1];
    ...
    initial begin
        for (init_i = 0; init_i < MAX_BEATS_ROW; init_i = init_i + 1) begin
            ...
            line_buf_3[init_i] = {INPUT_WIDTH{1'b0}};
        end
    end
    ...
    reg [INPUT_WIDTH-1:0] rd_data_3_r;
    ...
    always @(posedge clk) begin
        if (wr_en) begin
            case (wr_row_sel)
                ...
                2'd3: line_buf_3[wr_addr] <= wr_data;
    ...
    always @(posedge clk) begin
        ...
        rd_data_3_r <= line_buf_3[rd_addr];
    end
