...
* $ q_{a,j} $ – INT8 activations
* $ q_{w,cj} $ – INT8 weights for output channel `c`
...
* $ \max(|0.20|) = 0.20 \Rightarrow s_{w,0} = 0.20 / 127 \approx 0.001575 $
* $ q_{w,0} = \text{round}(0.20 / 0.001575) \approx \text{round}(127.0) = 127 $

For channel 1:

* $ \max(|-0.06|) = 0.06 \Rightarrow s_{w,1} = 0.06 / 127 \approx 0.000472 $
* $ q_{w,1} = \text{round}(-0.06 / 0.000472) \approx \text{round}(-127.1) = -127 $
...
To evaluate INT8 behavior *without* changing the actual model weights, `attach_qdq_hooks` registers forward hooks that replace module outputs with their QDQ versions (`qdq(y, s_a)`), effectively simulating activation quantization while keeping weights FP.
