...
def register_dumps(model: nn.Module, out_dir: str, quant_scales=None):
    ...
    This does NOT change the forward path; it is only for dumping. [attached_file:2]
    """
    ...
    for n, m in model.named_modules():
        if isinstance(m, (nn.Conv2d, nn.Linear, nn.GELU,
...
def accuracy(output, target, topk=(1, 5)):
    """
    Compute top-k accuracy for each k in topk. [attached_file:2]
    """
    maxk = max(topk)
...
def evaluate(model: nn.Module, loader: DataLoader):
    """
    Run evaluation over a dataset loader and return (top1, top5) in %. [attached_file:2]
    """
    model.eval()
...
def main():
    ...
    ap.add_argument('--img', default=None,
                    help='Optional: single image path for feature dumps')
    ...
    if args.val_dir is not None:
        ...
        # INT8 simulated via QDQ hooks (if scales provided)
        if qscales is not None:
            ...
            print(f'Loss:   top1 = {top1_fp - top1_q:.3f}%, top5 = {top5_fp - top5_q:.3f}%')
        else:
            print('No scales.json provided; skipping INT8(QDQ) evaluation.')
    ...
    if args.img is not None and args.dump_dir is not None:
        ...
        np.save(os.path.join(dump_dir, 'probs_topk_scores.npy'), topk.values.cpu().numpy())
        np.save(os.path.join(dump_dir, 'probs_topk_indices.npy'), topk.indices.cpu().numpy())
